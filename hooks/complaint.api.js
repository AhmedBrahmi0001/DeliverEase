...
const updateComplaintModel = async ({ complaintId, values }) => {
  const response = await axiosClient.post(
    `complaints/${complaintId}?_method=PUT`,
    values
  );
  return response;
};
...
const useUpdateComplaintModel = () => {
  const mutationConfig = {
    mutationFn: ({ complaintId, values }) =>
      updateComplaintModel({ complaintId, values }),
  };
...
export {
  useDeleteComplaintModel,
  useUpdateComplaintModel,
  useComplaintModels,
  useCreateComplaintModel,
  useGetComplaintModel,
};
