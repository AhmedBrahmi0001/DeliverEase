...
const updateNotificationModel = async ({ notificationId, values }) => {
  const response = await axiosClient.post(
    `notifications/${notificationId}?_method=PUT`,
    values
  );
  return response;
};
...
const useUpdateNotificationModel = () => {
  const mutationConfig = {
    mutationFn: ({ notificationId, values }) =>
      updateNotificationModel({ notificationId, values }),
  };
...
const markNotificationAsRead = async ({ notificationId }) => {
  const response = await axiosClient.post(
    `notifications/${notificationId}/mark-as-read?_method=PUT`
  );
